...
export default function BlogPage() {
  const posts = db.posts.findPublished().sort((a, b) =>
    ...
    <div style={{ minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <div style={{ fontFamily: 'Space Mono', color: 'var(--accent)', fontSize: 12, marginBottom: 12 }}>// BLOG</div>
          <h1 style={{ fontFamily: 'Syne', fontWeight: 800, fontSize: 'clamp(32px, 5vw, 48px)', marginBottom: 16 }}>
            Articles & Thoughts
          </h1>
          <p style={{ color: 'var(--muted)', fontSize: 17 }}>
            Writing about web development, architecture, and engineering practices.
          </p>
        </div>
      </div>
            ...
            {posts.map((post) => (
              <Link key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                <article className="card card-glow" style={{ padding: '28px 32px', cursor: 'pointer' }}>
                  ...
                  <h2 style={{ fontFamily: 'Syne', fontWeight: 700, fontSize: 22, marginBottom: 10, color: 'var(--text)' }}>
                    {post.title}
                  </h2>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <div style={{ display: 'flex', gap: 16 }}>
                      <span style={{ fontFamily: 'Space Mono', fontSize: 12, color: 'var(--muted)' }}>{post.views} views</span>
