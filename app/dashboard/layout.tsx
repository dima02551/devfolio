  ...
  if (!user) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
        <div style={{ fontFamily: 'Space Mono', color: 'var(--accent)', fontSize: 14 }}>Loading...</div>
      </div>
    );
  }
      ...
      <aside style={{
        ...
        <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid var(--border)' }}>
          <Link href="/" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 32, height: 32, background: 'linear-gradient(135deg, var(--accent), var(--accent2))', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#000', fontFamily: 'Space Mono' }}>
              {'<>'}
            </div>
        ...
        <nav style={{ flex: 1, padding: '16px 12px' }}>
          <div style={{ marginBottom: 4, padding: '4px 8px', fontFamily: 'Space Mono', fontSize: 10, color: 'var(--muted)' }}>NAVIGATION</div>
          {navItems.map((item) => {
          ...
          <div style={{ marginTop: 16, padding: '4px 8px', fontFamily: 'Space Mono', fontSize: 10, color: 'var(--muted)' }}>PORTFOLIO</div>
        ...
        <div style={{ padding: '16px', borderTop: '1px solid var(--border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
            <div style={{ width: 36, height: 36, background: 'linear-gradient(135deg, var(--accent2), var(--accent))', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, fontWeight: 700, color: '#fff', flexShrink: 0 }}>
          ...
          <button onClick={handleLogout} className="btn-ghost" style={{ width: '100%', padding: '7px', fontSize: 13, textAlign: 'center' }}>
            Sign Out
          </button>
        </div>
      </aside>
        ...
        <header style={{ borderBottom: '1px solid var(--border)', padding: '0 32px', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(5,8,16,0.8)', backdropFilter: 'blur(8px)', position: 'sticky', top: 0, zIndex: 30 }}>
          <div style={{ fontFamily: 'Space Mono', fontSize: 12, color: 'var(--muted)' }}>
            {pathname.split('/').filter(Boolean).map((p, i, arr) => (
              <span key={p}>
                <span style={{ color: i === arr.length - 1 ? 'var(--text)' : 'var(--muted)', textTransform: 'capitalize' }}>{p}</span>
                {i < arr.length - 1 && <span style={{ color: 'var(--border2)', margin: '0 6px' }}>/</span>}
              </span>
            ))}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--green)', boxShadow: '0 0 8px var(--green)' }} />
            <span style={{ fontFamily: 'Space Mono', fontSize: 11, color: 'var(--muted)' }}>LIVE</span>
          </div>
        </header>
